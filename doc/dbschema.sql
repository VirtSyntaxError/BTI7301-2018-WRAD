...
DROP TABLE IF EXISTS WRADEventType;
DROP TABLE IF EXISTS WRADEvent;
...
(
  ...
  GroupObjectGUID VARCHAR(36) NOT NULL,
  PRIMARY KEY (UserObjectGUID, GroupObjectGUID),
  CONSTRAINT `FK_RefUser` FOREIGN KEY (UserObjectGUID) REFERENCES WRADRefUser(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_RefGroup` FOREIGN KEY (GroupObjectGUID) REFERENCES WRADRefGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE
);
/*
...
(
  ...
  ParentGroupObjectGUID VARCHAR(36) NOT NULL,
  PRIMARY KEY (ChildGroupObjectGUID, ParentGroupObjectGUID),
  CONSTRAINT `FK_RefChildGroup` FOREIGN KEY (ChildGroupObjectGUID) REFERENCES WRADRefGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_RefParentGroup` FOREIGN KEY (ParentGroupObjectGUID) REFERENCES WRADRefGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE
);
/*
...
CREATE TABLE WRADEventType
(
  EventTypeID INT NOT NULL AUTO_INCREMENT,
  EventTypeText TEXT NOT NULL,
  PRIMARY KEY (EventTypeID)
);

CREATE TABLE WRADEvent
(
  EventID INT NOT NULL AUTO_INCREMENT,
  CreatedDate TIMESTAMP NULL,
  ResolvedDate TIMESTAMP NULL,
  SrcUserObjectGUID VARCHAR(36) NULL,
  SrcGroupObjectGUID VARCHAR(36) NULL,
  SrcRefUserObjectGUID VARCHAR(36) NULL,
  SrcRefGroupObjectGUID VARCHAR(36) NULL,
  DestGroupObjectGUID VARCHAR(36) NULL,
  ...
  CONSTRAINT `FK_EventSrcUser` FOREIGN KEY (SrcUserObjectGUID) REFERENCES WRADUser(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_EventSrcGroup` FOREIGN KEY (SrcGroupObjectGUID) REFERENCES WRADGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_EventSrcRefUser` FOREIGN KEY (SrcRefUserObjectGUID) REFERENCES WRADRefUser(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_EventSrcRefGroup` FOREIGN KEY (SrcRefGroupObjectGUID) REFERENCES WRADRefGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_EventDestGroup` FOREIGN KEY (DestGroupObjectGUID) REFERENCES WRADGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_EventDestRefGroup` FOREIGN KEY (DestRefGroupObjectGUID) REFERENCES WRADRefGroup(ObjectGUID) ON DELETE CASCADE ON UPDATE CASCADE
);
...
CREATE TRIGGER RefUserInsert
  ...
  FOR EACH ROW BEGIN
	...
	IF NEW.ObjectGUID = ''
	THEN 
	SET NEW.ObjectGUID = CONCAT('noguid', CAST(UNIX_TIMESTAMP(CURTIME(3))*1000 AS unsigned));
	END IF;
 END
//
/*
